...
class PdataS{
    ...
    {
        $sql="SELECT u.id,u.kifcim,u.username,p.userid,SUM(p.satoshi) AS egyenleg FROM userek u  INNER JOIN penztar p ON p.userid=u.id GROUP BY p.userid HAVING egyenleg>".SDT::$kif_limit;
...
class Admin extends AdminBase
{
    ...
    public function pub()
    {
        if (isset($_POST['sor']))
        {
            ...
            {
               $usersor= PdataS::user_leker($userid);
                if(!empty($usersor['kifcim']))
                {
                 ...
                 if(PdataS::utal(SDT::$kif_account_id,$formazott ,$usersor['kifcim'],'bejovo'))
                  {
                        $sql = "INSERT INTO penztar (userid,tr_cim,satoshi,megjegyzes)VALUES('" . $usersor['id']. "','kifizetés','-" . $usersor['egyenleg'] . "','kuldo accountid:" . SDT::$kif_account_id. "')";
                        // echo $sql;
                        DB::parancs($sql)   ;
